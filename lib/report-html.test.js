...
describe('renderReportHtml', () => {
  ...
  test('sections with a running command are expanded and float above both failing and clean sections', () => {
    const html = renderReportHtml({
      success: null,
      timestamp: 't',
      sections: [
        ...
        { title: 'SecFail', success: false, commands: [{ command: 'boom', success: false, durationMs: 1 }] },
        {
          title: 'SecRunning',
          success: null,
          commands: [{ command: 'inflight', success: null, startedAt: '2026-06-17T00:00:00.000Z' }],
        },
      ],
    });
    // the running section carries the has-running marker and is open
    expect(html).toContain('class="section has-running" open');
    // running sorts ahead of both failed and clean sections — check within the sections area
    // itself, since the Failures summary above it also mentions the failing section/command
    const sectionsStart = html.indexOf('<details class="section');
    expect(html.indexOf('SecRunning', sectionsStart)).toBeLessThan(html.indexOf('SecFail', sectionsStart));
    expect(html.indexOf('SecFail', sectionsStart)).toBeLessThan(html.indexOf('SecClean', sectionsStart));
  });

  test('a running row floats above a failed row in the same commands table', () => {
    const html = renderReportHtml({
      success: null,
      timestamp: 't',
      commands: [
        { command: 'rowfailer', success: false, durationMs: 1 },
        { command: 'rowrunning', success: null, startedAt: '2026-06-17T00:00:00.000Z' },
        { command: 'rowpasser', success: true, durationMs: 1 },
      ],
    });
    // check within the "Commands" table itself, since the Failures summary above it also
    // lists the failing row
    const tableStart = html.indexOf('<h3>Commands</h3>');
    expect(html.indexOf('<code>rowrunning</code>', tableStart)).toBeLessThan(html.indexOf('<code>rowfailer</code>', tableStart));
    expect(html.indexOf('<code>rowfailer</code>', tableStart)).toBeLessThan(html.indexOf('<code>rowpasser</code>', tableStart));
  });

  test('"only failures" filter is styled to keep running sections visible, not just failing ones', () => {
    const html = renderReportHtml({
      success: false,
      timestamp: 't',
      sections: [
        {
          title: 'SecFail',
          success: false,
          commands: [{ command: 'boom', success: false, durationMs: 1 }],
        },
        {
          title: 'SecRunning',
          success: null,
    ...
    // filter is offered (there is a real failure) and the running section carries the marker
    // the CSS selector must exempt, so it survives the "only failures" filter alongside SecFail
    expect(html).toContain('id="only-failures"');
    expect(html).toContain('class="section has-running" open');
    expect(html).toContain('details.section:not(.has-failure):not(.has-running) { display: none; }');
  });
