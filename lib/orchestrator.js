...
export class Orchestrator {
  ...
  async run() {
    ...
    try {
      ...
      // Stop periodic ticks + memory watchdog on error.
  ...
  async _abortOnMemoryPressure(info) {
    this.logger.error(
      `\n🛑 [memory-guard] ABORTING: host free RAM stayed at ~${info.freePercent}% for ${Math.round(info.sustainedMs / 1000)}s — ` +
    ...
    // Best-effort: persist what completed so the report isn't lost, and mark the run ended.
    try {
      this._writePartialResults();
    } catch {
