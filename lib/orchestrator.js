...
import { emitOtelTrace } from './otel/emit-trace.js';
...
export class Orchestrator {
  constructor(
    ...
    // Declarative OTEL tracing (set from config in index.js). When non-null, `otel: true | {...}`
    // fills in default file-exporter env vars at startup and, at run end, turns this gate's
    // computed `commands` array into an OTEL trace — one root span for the gate, one child span
    // per command, using each command's real startedAt/durationMs. See lib/otel/.
    this.otelOptions = null;
    this._lastResultCommands = null;
  ...
  writeJsonResults(hasFailures) {
    ...
    // Kept for the in-process OTEL trace emission (_emitOtelTrace) — same entries this payload
    // writes, so tracing needs no re-read of the json_results file.
    this._lastResultCommands = commands;
    ...
    try {
      ...
      if (this.startTime && this.metrics.includes('time')) {
        ...
        this.logger.printMessage(() =>
          console.log(
            chalk.cyan(
              `${this.logger.timestamp()} [INFO] ⏱️  Overall time taken: ${this.formatDuration(overallDuration)}`,
            ),
          ),
        );
      }
      ...
      // emit this gate's OTEL trace, if enabled
      await this._emitOtelTrace(hasFailures);
  ...
  // Turn this gate's just-computed `commands` array into an OTEL trace (lib/otel/emit-trace.js).
  // Non-fatal per the same contract as `_runPostRunHook`: any failure here only warns, never
  // affecting the run's own exit code.
  async _emitOtelTrace(hasFailures) {
    if (!this.otelOptions) return;
    try {
      const workspaceLabel = path.basename(process.cwd());
      const gateLabel =
        (this.jsonResultsPath && this.jsonResultsPath !== '-'
          ? path
            .basename(this.jsonResultsPath)
            .replace(/\.json$/, '')
            .replace(/^scripts-orchestrator-?/, '')
            .replace(/-?results$/, '')
          : '') || 'run';
      const serviceName =
        this.otelOptions.serviceName ||
        process.env.OTEL_SERVICE_NAME ||
        `scripts-orchestrator:${workspaceLabel}`;
      const tracesPath =
        this.otelOptions.filePath ||
        process.env.OTEL_EXPORTER_FILE_PATH ||
        path.join(
          this.logFolder ? path.resolve(this.logFolder) : (findRepoRoot() ?? process.cwd()),
          '.otel-logs',
          'traces.json',
        );
      await emitOtelTrace({
        commands: this._lastResultCommands,
        gateLabel,
        serviceName,
        tracesPath,
        hasFailures,
      ...
      this.logger.warn(`[otel] trace emission failed: ${error.message}`);
    }
  }
}
