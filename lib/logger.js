...
class Logger {
  ...
  startTask(id, text) {
    if (this.isTTY) {
      ...
      this.activeTasks.set(
        id,
        chalk.cyan(`${this.timestamp()} [INFO] ⏳ Running: ${text}`),
      );
  ...
  startEphemeral(id, message) {
    ...
    if (this.isTTY) {
      this.clearActiveTasks();
      this.activeTasks.set(id, timestampedMessage);
      this.renderActiveTasks();
    } else {
      this.printMessage(() => console.log(timestampedMessage));
    }
  }
  ...
  stopEphemeral(id, finalMessage = '', isError = false) {
    ...
    if (finalMessage) {
      if (isError) {
        this.printMessage(() =>
          console.error(
            chalk.red(`${this.timestamp()} [ERROR] ${finalMessage}`),
          ),
        );
      } else {
        this.printMessage(() =>
          console.log(
            chalk.green(`${this.timestamp()} [SUCCESS] ${finalMessage}`),
          ),
        );
      }
    }
  ...
  stopPhase(phaseName, success, durationStr) {
    if (success) {
      this.printMessage(() =>
        console.log(
          chalk.green(
            `${this.timestamp()} [SUCCESS] ✅ Phase "${phaseName}" completed successfully ${durationStr}`,
          ),
        ),
      );
      ...
      this.printMessage(() =>
        console.error(
          chalk.red(
            `${this.timestamp()} [ERROR] ❌ Phase "${phaseName}" completed with failures ${durationStr}`,
          ),
        ),
      );
      // Add a blank line after phase logs for readability
  ...
  initializeLogFile() {
    try {
      ...
      this.writeToFile(
        `${this.timestamp()} [START] Orchestrator started at ${new Date().toISOString()}\n`,
      );
    } catch (error) {
  ...
  info(message) {
    ...
    this.printMessage(() =>
      console.log(chalk.blue(`${ts} [INFO] ${message}`)),
    );
    this.writeToFile(`${ts} [INFO] ${message}`);
  }

  success(message) {
    const ts = this.timestamp();
    this.printMessage(() =>
      console.log(chalk.green(`${ts} [SUCCESS] ${message}`)),
    );
    this.writeToFile(`${ts} [SUCCESS] ${message}`);
  }
  ...
  error(message) {
    ...
    this.writeToFile(`${ts} [ERROR] ${message}`);
  }
  ...
  warn(message) {
    ...
    this.writeToFile(`${ts} [WARN] ${message}`);
  }
  ...
  verbose(message) {
    if (this.isVerbose) {
      const ts = this.timestamp();
      this.printMessage(() =>
        console.log(chalk.gray(`${ts} [VERBOSE] ${message}`)),
      );
      this.writeToFile(`${ts} [VERBOSE] ${message}`);
    }
  }
}
