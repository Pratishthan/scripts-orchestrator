...
import { availableMemoryRatio } from './host-memory.js';
...
export class MemoryGovernor {
   ...
   * @param {Function} [deps.freeRatioFn] - () => current available-RAM fraction (0..1).
  ...
  constructor(options = {}, deps = {}) {
    ...
    // Default reads *available* RAM (free + reclaimable cache), cross-platform — not os.freemem(),
    // which excludes reclaimable pages and so reads ~0% during any healthy heavy build. See
    // host-memory.js.
    this._freeRatioFn = deps.freeRatioFn ?? availableMemoryRatio;
  ...
  /** Current available RAM as an integer percentage, for log lines. */
  freePercent() {
  ...
  async waitForHeadroom(inFlight) {
    ...
    this.logger.info(
      `🧠 [memory-guard] holding next command — available RAM ${this.freePercent()}% < admission floor ${floorPct}% (${inFlight} running)`,
    );
    ...
    while (this.freeRatio() < this.opts.minFreeRatio) {
      ...
      if (waited >= this.opts.maxHoldMs) {
        this.logger.warn(
          `🧠 [memory-guard] available RAM still ${this.freePercent()}% after holding ${Math.round(waited / 1000)}s; ` +
    ...
    this.logger.info(
      `🧠 [memory-guard] available RAM recovered to ${this.freePercent()}% — releasing held command.`,
    );
  }
  ...
  describe() {
    ...
    return (
      `memory-guard: hold next command below ${Math.round(this.opts.minFreeRatio * 100)}% available RAM, ` +
