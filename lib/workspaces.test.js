...
describe('aggregateWorkspacesReport', () => {
  ...
  // Write a terminal root results file as Orchestrator.finalizeInterrupted would on a kill.
  function interruptedRootResults(commands = []) {
    writeJson(
      path.join(repoRoot, 'logs/scripts-orchestrator-logs/scripts-orchestrator-results.json'),
      { success: false, interrupted: true, timestamp: fresh(5), overallDurationMs: 50, commands },
    );
  }

  test('a user-terminated run is non-success but flagged interrupted (amber, not Failed)', () => {
    interruptedRootResults([{ command: 'lint', phase: 'global quality checks', success: true }]);
    makeWorkspace('apps/web', {
      results: {
        success: null,
        timestamp: fresh(),
        commands: [{ command: 'test', success: null }], // left mid-flight by the kill
      },
    });

    const report = aggregateWorkspacesReport({ repoRoot });
    expect(report.success).toBe(false); // still non-success → CI gating unchanged
    expect(report.interrupted).toBe(true); // but distinguished from a real failure
    expect(report.sections.find((s) => s.title === 'apps/web').statusLabel).toBe('INTERRUPTED');
  });

  test('a genuine failure dominates a concurrent interruption (no interrupted flag)', () => {
    interruptedRootResults([{ command: 'lint', phase: 'global quality checks', success: true }]);
    makeWorkspace('apps/web', {
      results: { success: null, timestamp: fresh(), commands: [{ command: 'test', success: null }] },
    });
    makeWorkspace('packages/a', {
      results: { success: false, timestamp: fresh(), commands: [{ command: 'build', success: false }] },
    });

    const report = aggregateWorkspacesReport({ repoRoot });
    expect(report.success).toBe(false);
    expect(report.interrupted).toBeUndefined(); // a real FAIL outranks the interruption
  });

  test('a stale interrupted root file (prior run) does not flag a fresh run as interrupted', () => {
    // Old terminal file from a previous run + a clean current run → plain success, no interrupted.
    writeJson(
      path.join(repoRoot, 'logs/scripts-orchestrator-logs/scripts-orchestrator-results.json'),
      { success: false, interrupted: true, timestamp: old(), commands: [] },
    );
    makeWorkspace('apps/web', { results: { success: true, timestamp: fresh(), commands: [] } });
    markRunning(); // a run is in flight now
    const report = aggregateWorkspacesReport({ repoRoot, inProgress: false });
    expect(report.interrupted).toBeUndefined();
  });
  ...
  test('a stale FAILED lane is not surfaced under an otherwise-passing section', () => {
    // Regression: a later lane left a previous-run FAILURE behind (stale by timestamp) while the
    // earlier lanes passed this run. The stale failure must NOT leak in as a red command under an
    // OK/green section header (the contradictory "section OK but a leaf failed" report).
    rootResults([
      { command: 'lint', phase: 'global quality checks', success: true },
      { command: 'fan-out', phase: 'workspace quality gates', success: true },
    ]);
    // Lite lane: cache replay (stale) pass.
    makeWorkspace('apps/web', {
      results: {
        success: true,
        timestamp: old(600),
    ...
    // Storybook lane: ran fresh this run and passed.
    writeJson(
      path.join(repoRoot, 'apps/web/logs/scripts-orchestrator-logs/scripts-orchestrator-storybook-results.json'),
      { success: true, timestamp: fresh(), commands: [{ command: 'storybook', phase: 'storybook tests', success: true }] },
    );
    // Playwright lane: STALE leftover failure from a prior run (did not run this run).
    writeJson(
      path.join(repoRoot, 'apps/web/logs/scripts-orchestrator-logs/scripts-orchestrator-playwright-results.json'),
      { success: false, timestamp: old(600), commands: [{ command: 'playwright', phase: 'playwright tests', success: false }] },
    );
    ...
    const report = aggregateWorkspacesReport({
      ...
      workspaceResults: [
        'logs/scripts-orchestrator-logs/scripts-orchestrator-results.json',
        'logs/scripts-orchestrator-logs/scripts-orchestrator-storybook-results.json',
        'logs/scripts-orchestrator-logs/scripts-orchestrator-playwright-results.json',
      ],
    });
    const web = report.sections.find((s) => s.title === 'apps/web');
    expect(web.statusLabel).toBe('OK');
    expect(web.success).toBe(true);
    // The stale playwright failure is dropped; only the lanes that actually ran/passed remain.
    expect(web.commands.map((c) => c.command)).toEqual(['test-ci', 'storybook']);
  });

  test('an earlier fresh failure invalidates later stale lanes (fail-fast skipped stages)', () => {
    // The lite gate ran fresh and FAILED, so under fail-fast the Storybook/Playwright stages never
    // ran this run; their stale (previous-run) results must be invalidated, not backfilled.
    rootResults(
      [
        { command: 'lint', phase: 'global quality checks', success: true },
        { command: 'fan-out', phase: 'workspace quality gates', success: false },
    ...
    // Lite lane: ran fresh and failed.
    makeWorkspace('apps/web', {
      results: {
        success: false,
        timestamp: fresh(),
        commands: [{ command: 'build:check', phase: 'build', success: false }],
      },
    });
    // Storybook lane: stale PASS from a prior run — but this stage did not run this time.
    writeJson(
      path.join(repoRoot, 'apps/web/logs/scripts-orchestrator-logs/scripts-orchestrator-storybook-results.json'),
      { success: true, timestamp: old(600), commands: [{ command: 'storybook', phase: 'storybook tests', success: true }] },
    );
    ...
    const web = report.sections.find((s) => s.title === 'apps/web');
    expect(web.statusLabel).toBe('FAIL');
    // Only the fresh failing lane is surfaced; the stale storybook pass is invalidated.
    expect(web.commands.map((c) => c.command)).toEqual(['build:check']);
  });
  ...
  test('an interrupted workspace (terminal success:false + interrupted:true) reads INTERRUPTED, not FAIL', () => {
    // finalizeInterrupted() writes a TERMINAL success:false so the run reads as ended, but stamps
    // interrupted:true and leaves no command failed (passed ones stay true, in-flight ones null).
    // The roll-up must honour interrupted and render INTERRUPTED — not a misleading hard FAIL.
    rootResults(
      [{ command: 'lint', phase: 'global quality checks', success: true }],
      false,
    );
    ...
    makeWorkspace('apps/web', {
      results: {
        ...
        timestamp: fresh(),
        commands: [
          { command: 'type-check', success: true },
          { command: 'test-ci', success: null }, // cut off mid-flight
        ],
      },
    });
    const report = aggregateWorkspacesReport({ repoRoot });
    expect(report.inProgress).toBe(false);
    const ws = report.sections.find((s) => s.title === 'apps/web');
    expect(ws.statusLabel).toBe('INTERRUPTED');
    expect(ws.commands).toHaveLength(2); // partial list still surfaced
    expect(report.interrupted).toBe(true); // amber banner, not red Failed
    expect(report.success).toBe(false);
  });

  test('a genuinely-failed command in an interrupted workspace still reads FAIL (failure dominates)', () => {
    rootResults(
      [{ command: 'lint', phase: 'global quality checks', success: true }],
      false,
    );
    ...
    makeWorkspace('apps/web', {
      results: {
        ...
        timestamp: fresh(),
        commands: [
          { command: 'type-check', success: false }, // a real gate failure
          { command: 'test-ci', success: null },
        ],
      },
    });
    const report = aggregateWorkspacesReport({ repoRoot });
    expect(report.sections.find((s) => s.title === 'apps/web').statusLabel).toBe('FAIL');
    expect(report.interrupted).toBeUndefined(); // real failure outranks the interruption
    expect(report.success).toBe(false);
  });
