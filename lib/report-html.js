...
// Thresholds (fractions of the run's peak) are configurable; defaults are used otherwise.
// Returns null when no memory was recorded (heat accent is then omitted).
export function memoryHeat(memoryKb, maxMemory, thresholds = DEFAULT_HEAT_THRESHOLDS) {
  if (memoryKb == null || !(maxMemory > 0)) return null;
  const frac = memoryKb / maxMemory;
