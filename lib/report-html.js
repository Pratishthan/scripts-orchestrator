...
// Stable fail-first ordering: nodes containing a failure sort ahead of clean ones, order otherwise
// preserved. Generic — "failure" is only success === false, no domain knowledge.
function failFirst(nodes) {
  return [...nodes].sort((a, b) => (hasFailure(a) ? 0 : 1) - (hasFailure(b) ? 0 : 1));
}
...
function renderCommandsTable(commands, columns, ctx) {
  ...
  // Float failed commands to the top of the table so a failure never hides below passing rows.
  const ordered = [...commands].sort((a, b) => (a.success === false ? 0 : 1) - (b.success === false ? 0 : 1));
  const rows = ordered
    .map((c) => {
      const st = statusOf(c);
...
function renderSection(section, columns, ctx) {
  ...
  // Expand only sections that contain a failure; passing ones collapse to their one-line summary so
  // the reader isn't wading through green. `has-failure` lets the "only failures" filter keep them.
  const cls = `section${failed ? ' has-failure' : ''}`;
...
body.failures-only details.section:not(.has-failure) { display: none; }
