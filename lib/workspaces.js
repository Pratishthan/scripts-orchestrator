...
function readMergedWorkspaceResults(wsDir, workspaceResults, relDir, runStartedAt) {
  ...
  // A workspace whose own run was interrupted (SIGINT/SIGTERM/…) stamps `interrupted: true` into its
  // results JSON with a TERMINAL top-level `success: false` (see Orchestrator.finalizeInterrupted) so
  // the run reads as ENDED — even though no gate actually failed. Map such a payload to the
  // interrupted sentinel (`null`), NOT a failure, UNLESS one of its OWN commands genuinely failed
  // (success === false): a real gate failure dominates a concurrent cancellation — the same rule the
  // top-level roll-up applies (see `rootRealFail`). Without this, an interrupted workspace's
  // terminal `success: false` is read as a hard FAIL instead of INTERRUPTED.
  const effectiveSuccess = (payload) => {
    if (payload?.interrupted === true) {
      const cmds = Array.isArray(payload.commands) ? payload.commands : [];
      if (!cmds.some((c) => c?.success === false)) return null;
    }
  ...
  let success;
  if (fresh.length > 0) {
    if (fresh.some((p) => effectiveSuccess(p.payload) === false)) success = false;
    else if (fresh.some((p) => effectiveSuccess(p.payload) == null)) success = null;
    else success = true;
  }
  ...
  // Decide which present files may contribute commands. The lanes are processed IN THE ORDER they
  // are configured (`workspaceResults`), which is also their stage order — lite gate, then
  // Storybook, then Playwright. Two kinds of "stale by timestamp" must be told apart:
  //
  //   (a) Cache replay within THIS run — Nx served a lane's whole target from cache, so its
  //       orchestrator process never re-ran and never rewrote its results JSON. The file predates
  //       the run but is a VALID pass for it (Nx only caches successes). These must still be
  //       surfaced, else e.g. the entire lite gate vanishes when only the browser lanes run fresh.
  //   (b) A stage that DID NOT RUN this run — skipped because an EARLIER stage failed (the gate is
  //       a strict, fail-fast pipeline: a failed phase aborts the rest). Its JSON is genuine
  //       previous-run leftover; surfacing it would report stale information for work that never
  //       happened this run. A failure must INVALIDATE the rest of the report, not backfill it.
  //
  // So: a fresh lane always contributes. Once an earlier fresh lane has FAILED, every later stale
  // lane is invalidated (case b) — dropped, regardless of what its old file said. A stale lane that
  // is NOT behind a fresh failure contributes only when it is a trustworthy cached pass
  // (`success === true`); a stale FAILED/interrupted file is untrustworthy leftover and is dropped
  // too (it must never surface a red command under an otherwise-green section). The workspace's
  // pass/fail and CACHED/STALE/RUNNING classification still derive solely from the FRESH files
  // (`success`/`stale` above).
  let earlierFreshFailure = false;
  const usable = [];
  for (const p of present) {
    if (!p.stale) {
      usable.push(p);
      if (p.payload?.success === false) earlierFreshFailure = true;
      continue;
    }
    if (earlierFreshFailure) continue; // (b) earlier stage failed → this stage did not run
    if (p.payload?.success === true) usable.push(p); // (a) trustworthy cache replay
    // else: stale failure/interruption → untrustworthy leftover, dropped
  }
...
export function aggregateWorkspacesReport(options = {}) {
  ...
  let anyWorkspaceFail = false; // a genuine gate failure (≠ a user interruption)
  let anyWorkspaceInterrupted = false; // a command left mid-flight by a kill
  for (const wsDir of discoverWorkspaceDirs(opts.repoRoot)) {
    ...
    if (st.state === 'fail') anyWorkspaceFail = true;
    if (st.state === 'interrupted') anyWorkspaceInterrupted = true;
  ...
  // Distinguish a user-terminated run from a genuine failure for the overall banner. The root run
  // stamps `interrupted: true` into its results JSON when killed (see Orchestrator.finalizeInterrupted);
  // a stale leftover from a PRIOR run must not count, so only honour it on a fresh root file. The run
  // reads as "interrupted" only when there is NO genuine failure — a real gate failure dominates a
  // concurrent cancellation. `success` is unaffected (it stays false), so this only changes the label.
  const rootInterrupted =
    rootJson?.interrupted === true && !isStale(opts.rootResultsAbs, rootJson, runStartedAt);
  const rootRealFail = rootJson?.success === false && !rootInterrupted;
  const hasRealFailure = globalFailed > 0 || anyWorkspaceFail || rootRealFail;
  const interrupted =
    !inProgress && !hasRealFailure && (rootInterrupted || anyWorkspaceInterrupted);
  ...
  return {
    title: opts.title,
    success,
    // Non-success-but-not-a-failure: a user-terminated run. Renders an amber "Interrupted" banner
    // instead of red "Failed". Omitted when false so a normal report's JSON is unchanged.
...
function atomicWrite(absPath, content) {
  // Unique temp name per writer+call. On a group interrupt (SIGINT/SIGTERM to the whole tree) several
  // processes can fire the roll-up at once; a SHARED `${absPath}.tmp` then races — one process renames
  // it away and the other's rename fails with ENOENT, losing that write. A pid+seq-scoped temp keeps
  // each writer isolated; the rename onto the final path stays atomic (last writer wins).
  const tmp = `${absPath}.${process.pid}.${atomicWriteSeq++}.tmp`;
  fs.mkdirSync(path.dirname(absPath), { recursive: true });
  fs.writeFileSync(tmp, content, 'utf8');
  try {
    fs.renameSync(tmp, absPath);
  } catch (err) {
    // Best-effort cleanup of our own temp so a failed rename doesn't litter the logs dir.
    try {
      fs.unlinkSync(tmp);
    } catch {
      /* ignore */
