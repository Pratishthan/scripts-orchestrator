...
function readMergedWorkspaceResults(wsDir, workspaceResults, relDir, runStartedAt) {
  ...
  // Prefer fresh files; when every present file is stale, still surface the last-known
  // (cached) commands so a CACHED workspace shows its prior results rather than "no data".
  const usable = fresh.length > 0 ? fresh : present;
  ...
  return { exists, stale, success, commands, overallDurationMs };
}
...
export function aggregateWorkspacesReport(options = {}) {
  ...
  for (const wsDir of discoverWorkspaceDirs(opts.repoRoot)) {
    ...
    const { exists, stale, success } = merged;
